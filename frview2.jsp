...
<%@ page session="true" %>
  ...
  //String mobile = request.getParameter("bccode"); 
    ...
    int mobile = psid.hashCode();
    int k = 0;
    
    try{   
        
    Connection con=Dbconnection.getConnection();
    PreparedStatement ps=con.prepareStatement("insert into frreport values(?,?,?,?,?)");
    //ps.setInt(1,k);
  ...
  ps.setString(5,psid);
    ...
    if(i>0)
    {
    response.sendRedirect("frview.jsp?m1=Registered");
    }
    else{
    response.sendRedirect("frview.jsp?m2=Failed");    
    }
    %>
    <%
    }
