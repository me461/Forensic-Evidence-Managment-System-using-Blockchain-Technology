...
if(request.getParameter("msg")!=null){%>
    ...
    out.print("<script>alert('Request_sent_to_Cloud2..!')</script>");
}  

<%}
if(request.getParameter("msgg")!=null){%>

 out.print("<script>alert('Request_failed..!')</script>");
 <%
}
%>
<%
    ...
    <div id="templatemo_menu" class="ddsmoothmenu">
        <ul>
            <li><a href="hospital2home.jsp">Home</a></li>
                                     <li><a href="fupload1.jsp">File Upload</a></li>
                                    <li><a href="sendtocloud1.jsp" class="selected">Send to Cloud</a></li>                                    
...
to cloud.</p>
    ...
    <div id="templatemo_main">
        <center>
        	      <h2><font color="white">Send Files to Cloud </font></h2>
...
try{ 
	...
	PreparedStatement pst=con.prepareStatement("select * from hfiles where hospital = 'hospital2'");
  ...
  <th><font color="#ff8080"> Dna1</th>
    <th><font color="#ff8080"> Dna2</th>
      <th><font color="#ff8080"> Dna3</th>
        <th><font color="#ff8080"> Dna4</th>
     <th><font color="#ff8080"> Date</font></th>
...
<%
 ...
 while(rs.next()){
            %> <center>  <tr><td> <center><%=rs.getString(2)%></td>
                    <td> <center><%=rs.getString(3)%></td>
                    <td> <center><%=rs.getString(4)%></td>
                    <td> <center><%=rs.getString(5)%></td>
                    <td> <center><%=rs.getString(6)%></td>
                <td> <center><%=rs.getString(7)%></td>
             
               
              
                <th><a href="sendtocloudact1.jsp?pid=<%=rs.getString("pid")%>&dna1=<%=rs.getString("dna1")%>&dna2=<%=rs.getString("dna2")%>&dna3=<%=rs.getString("dna3")%>&dna4=<%=rs.getString("dna4")%> &date=<%=rs.getString("date")%>">   Send</a> </th>
